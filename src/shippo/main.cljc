(ns shippo.main
  "Kotodama WASM entrypoint for the Shippo clean-room actor (L4) — Clojure port.
  ...
  facts (`shippo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "shippo")
(def tier "L4")
...
(def entity-specs
  [{:entity "Address"      :plural "addresses"      :id-prefix "shippo_add"
    :fields [:name :company :street1 :city :state :zip :country :phone :email :isResidential]
    :required [:name :company]
    :coerce {:isResidential :bool}    :refs {}}
   {:entity "Parcel"       :plural "parcels"        :id-prefix "shippo_par"
    :fields [:objectId :weight :massUnit :length :width :height :distanceUnit :objectCreated]
    :required [:weight :massUnit]
    :coerce {}                        :refs {}}
   {:entity "Shipment"     :plural "shipments"      :id-prefix "shippo_shi"
    :fields [:objectId :status :addressFrom :addressTo :shipmentDate :objectCreated]
    :required [:status :addressFrom]
    :coerce {}                        :refs {}}
   {:entity "Rate"         :plural "rates"          :id-prefix "shippo_rat"
    :fields [:objectId :amount :currency :carrierAccount :servicelevelToken :estimatedDays :objectCreated]
    :required [:amount :currency]
    :coerce {:estimatedDays :int}     :refs {}}
   {:entity "Transaction"  :plural "transactions"   :id-prefix "shippo_tra"
    :fields [:objectId :status :objectState :trackingNumber :trackingStatus :labelUrl :labelFileType :rate :test :objectCreated]
    :required [:status :objectState]
    :coerce {:test :bool}             :refs {}}
   {:entity "TrackingStatus" :plural "trackingstatuses" :id-prefix "shippo_tra"
    :fields [:objectId :status :statusDetails :substatus :statusDate]
...
(defn emit-facts
  "EAVT facts for one record: {\"shippo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "shippo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :shippo-compat/ready)
